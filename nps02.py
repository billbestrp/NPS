#!/usr/bin/env python3
"""
NPS02.py - Now Playing Script v2.0
Monitors a text file for changes and immediately posts artist/title data to RadioPlayer API.
Uses X-API-KEY header authentication instead of basic authentication.
"""
...
class RadioPlayerAPI:
    """Handles API communication with RadioPlayer service using X-API-KEY authentication."""
    
    def __init__(self, config: Config):
        self.config = config
        self.session = requests.Session()
        # Set API key in headers instead of basic auth
        self.session.headers.update({
            'X-API-KEY': config.api_key,
            'User-Agent': 'NPS02.py/2.0'
        })
    ...
    def post_now_playing(self, artist: str, title: str) -> bool:
        ...
        try:
            ...
            logging.debug(f"Headers: X-API-KEY={self.config.api_key[:8]}***")
...
class RadioPlayerMonitor:
    ...
    def start_monitoring(self):
        ...
        try:
            ...
            logging.info("Using X-API-KEY authentication")
